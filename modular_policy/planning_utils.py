...
Utilities for the trajectory optimization (motion planning) phase
'''
...
## Or, no tripod penalty, can discover its own gait style:
w_tripod = 0
w_center = 3
...
class fd_func_autodiff(torch.nn.Module):

 def __init__(self, network_options, module_sa_len, attachments,
             device, fd_networks, modules_set, gnn_nodes_set):
    super(fd_func_autodiff, self).__init__()
    ...
    self.fd_networks = fd_networks
    self.modules_set = modules_set
    self.gnn_nodes_set = gnn_nodes_set
    self.device = device
 def forward(self, state, action):
     ...
     n_networks = self.network_options['n_ensemble']
     ...
     if self.network_options['frame'] == 'Body':
         fd_input, R0_t = state_to_fd_input(state)
     else:
         fd_input = state
#             xy_real = fd_input[0][:,0:2]
#             fd_input[0][:,0:2] -=xy_real
         
     delta_fd_list = []
     delta_fd_var_list = []


     if self.network_options['type'] == 'MLP':
         fd_input = torch.cat(fd_input,1)
         actions_in = torch.cat(action,1)
         for i in range(n_networks):
             fd_network = self.fd_networks[i]
             if network_options['output'] == 'Probabilistic':
                 ...
                 state_delta_est_mean = fd_network(fd_input, actions_in)
                 delta_fd_list.append(state_delta_est_mean)
                 delta_fd_var_list.append(torch.zeros_like(state_delta_est_mean))

     else: # GNN
         actions_in = action
         for i in range(n_networks):
             modules = self.modules_set[i]
             gnn_nodes = self.gnn_nodes_set[i]
             ...
             if self.network_options['output'] == 'Probabilistic':
                 state_delta_est_mean, state_delta_est_var = pgnn.run_propagations(
                     modules, self.attachments, 2, node_inputs, self.device)
                 state_delta_est_mean = combine_state(state_delta_est_mean)
                 state_delta_est_var = combine_state(state_delta_est_var)
                 delta_fd_list.append(state_delta_est_mean)
                 delta_fd_var_list.append(state_delta_est_var)  
             else:
                 state_delta_est_mean = gnn.run_propagations(
                     modules, self.attachments, 2, node_inputs, self.device)
                 state_delta_est_mean = combine_state(state_delta_est_mean)
                 delta_fd_list.append(state_delta_est_mean)
                 delta_fd_var_list.append(torch.zeros_like(state_delta_est_mean))
     ...
     if self.network_options['frame'] == 'Body':
         ...
         state_next_approx = state_add_batch(state, delta_fd_approx)
...
class fd_func_jac(torch.nn.Module):


 def __init__(self, network_options, module_sa_len, attachments,
             device, fd_networks, modules_set, gnn_nodes_set,
             finite_diff_delta = 1e-4):
    super(fd_func_jac, self).__init__()
    self.network_options = network_options
    self.module_state_len = module_sa_len[:int(len(module_sa_len)/2)]
    self.module_action_len = module_sa_len[int(len(module_sa_len)/2):]
    self.attachments = attachments
...
# uses center differencing
# TODO: can batch all finite differencing into one big forward pass.
class fd_func_jac2(torch.nn.Module):

 def __init__(self, network_options, module_sa_len, attachments,
             device, fd_networks, modules_set, gnn_nodes_set,
             finite_diff_delta = 1e-4):
    super(fd_func_jac2, self).__init__()
    self.network_options = network_options
    self.module_state_len = module_sa_len[:int(len(module_sa_len)/2)]
    self.module_action_len = module_sa_len[int(len(module_sa_len)/2):]
    self.attachments = attachments
 ...
 def grad_input(self, state, action):
     ...
     action_rep = action.repeat(n_ctrl,1)
     eye_rep_ctrl = torch.eye(n_ctrl,device=device).repeat_interleave(batch_size, dim=0)
     action_perturbed2 = action_rep - eye_rep_ctrl*finite_diff_delta
     state_next_a_perturbed2 = self.forward(state_rep, action_perturbed2)
     ...
     action_rep = action.repeat(n_state,1)
     eye_rep_state = torch.eye(n_state,device=device).repeat_interleave(batch_size, dim=0)
     state_perturbed2 = state_rep - eye_rep_state*finite_diff_delta
     state_next_s_perturbed2 = self.forward(state_perturbed2, action_rep)


     # center differencing
     # compute with non-perturbed states and actions
     state_next = self.forward(state, action)
     delta_ss = (state_next_s_perturbed - state_next_s_perturbed2)/(2*finite_diff_delta)
     delta_sa = (state_next_a_perturbed - state_next_a_perturbed2)/(2*finite_diff_delta)
