...
This script runs inference 500 time steps with random goals without doing and simulation,
to get a measurement of the compute time taken by the policy. 
...
step = 0
n_steps = 500
desired_xyyaws=  np.random.uniform(low=-1, high=1, size= [n_steps, 3 ])

# Only measures state once
env_state = env.get_state()
...
while step<n_steps:
    ...
    desired_xyyaw = desired_xyyaws[step] # picks a new random goal each time
    ...
    # env.step(action) # does not actually simulate
    
    
    last_states = [s.clone() for s in states]
    step+=1

dt_measured = (time.time() - start_time)/n_steps*1000
print(urdf_name + ' Time elapsed: ' + str(dt_measured) + ' ms')