...
mbrl.py is the main file that runs the modular policy learning process via phases:
...
3) plan with learned model to generate mpc rollout data. 
 -- uses policy to generate intitial seed
4) learn to clone plans into policy
5) simulate policy to validate and gather policy rollout data
- return to (2) 
- repeat

It keeps the current network weights in cpu, then
transfers them to gpu as needed, e.g. at the start of training,
or when spawning a new process that can benefit from gpu.
...
import gc, os
import logging
from MPC_batch_policy import plan_batch
from train_multidesign_control import train_control
...
if __name__ == '__main__':
    ...
    # machine if this is set to true.
    ...
    # Change the trial name here to avoid overwriting old trials
    folder = os.path.join(cwd, 'saved/no_tripod_trial2')   


    if USE_MULTIPROCESS:
        # # set pytorch to spawn processes
    ...
    del env # remove the environment to save memory, since mbrl.py does not use them anymore
    ...
    for urdf in urdf_names:
        ...
        # if False: # force to recreate it
            ...
            if USE_MULTIPROCESS: # Multiprocess version
                ...
                # Spawn processes to collect random action rollouts in parallel on CPU
                processes = []
                for p_num in range(num_processes):
            ...
            else: # # single process verion, if multiprocess is not available. This will take much longer.
    ...
    for p in pgnn.get_GNN_params_list(gnn_nodes_model):
        ...
        for s in list(p.size()):
            nn = nn*s
        num_nn_params += nn
    logging.info('Num NN params: ' + str(num_nn_params))

    # Train the initial dynamics model approximation
    ...
    ## Create policy GNN
    control_save_path = os.path.join(folder, 'multidesign_control_iter0.pt')
    ...
    # Graph neural network initialize
    goal_len = 3
    ...
    for node in gnn_nodes_control:
        node.share_memory() # needed for multiprocessing
...
#### ----- Main MBRL loop ----- ####
    ...
    for mbrl_iter in range(1,num_mbrl_iters+1):
        ...
        if not(USE_MULTIPROCESS):# or len(urdf_names)==1:
            ...
            # One might be able to distribute more efficiently later
            # to use multiple GPUS efficiently, or
            #  to handle gpu memory running out inside spawned processes
        ...
        with open(vel_save_path, 'w') as fp:
            ...
            # Plot all the velocity measurement metrics for debug
        ...
        # (unless this was the last iteration, in which case no need)
            ...
            if os.path.exists(model_fname):
                ...
                frac_new_data = 0.1 # force some fraction of the batch to use the newer data
        ...
        if mbrl_iter == 1 :      
            ...
            # Takes some extra steps in first iteration to warm up the gnn
