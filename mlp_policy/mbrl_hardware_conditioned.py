...
Run mbrl, with **hardware-conditioned architecture** for model and policy:
...
import gc, os
import logging
from shared_MLP_model import hardware_conditioned_model
...
if __name__ == '__main__':
    ...
    folder = os.path.join(cwd, 'saved/hc_tripod_trial2')   


    if USE_MULTIPROCESS:
    # # set to spawn processes for multiprocess
    ...
    # check if file exists, if so load it.
    model_fname = 'hardware_conditioned_ms'+ str(int(seq_len))+ '_iter' + str(int(mbrl_iter)) + '.pt'
    ...
    ## Variable lengths needed to define hardware-conditioned network sizes/zero-padding
    ...
    weight_decay = 1e-4
    lr_init = 1e-3
    model_network = hardware_conditioned_model(
    ...
    ## Create control GNN
    control_save_path = 'hardware_conditioned_control_iter0.pt'
    ...
    for mbrl_iter in range(1,num_mbrl_iters+1):
        ...
        if not(USE_MULTIPROCESS):# or len(urdf_names)==1:
            ...
            if torch.cuda.device_count()==4: # for quad workstation:
                ...
                devices = [ torch.device('cuda:0') ,torch.device('cuda:1') ,torch.device('cuda:2') ]*2 
            else:
            ...
            # n_runs_per_process = 3 ## For debug only, small number of runs
        ...
        for urdf in urdf_names:
            ...
            # logging.info(urdf + ' vel metric baseline: ' + str(vm_baseline))
        ...
        if mbrl_iter == 1 :      
            n_training_steps_control += 1000 # takes some steps to warm up the gnn
        ...
        control_save_path = 'hardware_conditioned_control_iter' + str(int(mbrl_iter))+'.pt'
        control_save_path = os.path.join(folder, control_save_path)
        ...
        # First make some direction goals
