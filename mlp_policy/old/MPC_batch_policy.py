...
import logging # deos this go here?
...
def plan_batch(worker_num, urdf, gnn_nodes_model_in, gnn_nodes_control_in, 
    ...
    print('Starting ' + os.path.realpath(__file__) + ' ' + urdf +' ' +  
    ...
    # test_goals = [[speed_scale_xy*0.5,0,0],
    #               [0,speed_scale_xy*0.5,0],
    #               [0,0,speed_scale_yaw*0.25],
    #               [-speed_scale_xy*0.1,0,0],
    #               [0,-speed_scale_xy*0.1,0],
    #               [0,0,-speed_scale_yaw*0.1]]
    ...
    for i_traj in range(n_runs):
        ...
        try: # if an error happens, want to move on to next i_traj
            ...
            turn = np.clip(np.random.normal(0,0.25*speed_scale_yaw, 
                                        size=batch_size),
            ...
            if len(test_goals)>0:
                for ib in range(batch_size):
                    if len(test_goals)>0:
                        test_goal = np.array(test_goals.pop(0))
            ...
            for i in range(batch_size):
                ...
                if len(states_memory)>0 and not(used_test_goals) and len(sampleable_runs)>0:
                    try:
                        ...
                        # ind_mem2 = np.random.choice(states_memory[ind_mem1][0].shape[0]-T) # index
            ...
            for replan in range(n_replans):
                ...
                if use_policy:
                    ...
                    state_approx = [torch.tensor( np.stack(s),dtype=torch.float32, device=device)
                             ...
                             # [ stacked_module1_data , stacked_module2_data...]
                    # print(states_for_policy)
                    # print(state_approx)
                    # print(len(state_approx))
                    # print('running policy forward')
                    # print([list(state_approx[i].shape) for i in range(len(state_approx))])
                    ...
                    for t in range(T):
                        ...
                        node_inputs_control = create_control_inputs(state_approx, goals_world)
                ...
                try:
                    ...
                    for i in range(batch_size):  
                        xyyaw_start = [envs[i].pos_xyz[0], envs[i].pos_xyz[1], envs[i].pos_rpy[2]]
                        ...
                        for t in range(n_execute):
                            if robot_alive[i]:
                                ...
                                if np.dot([0,0,1], envs[i].z_axis)<0:
                                    robot_alive[i] = False
                        if show_GUI:
                            xyyaw_end = [envs[i].pos_xyz[0], envs[i].pos_xyz[1], envs[i].pos_rpy[2]]
                            print('Env' + str(i) + ' delta x y yaw: ' 
                                + str(xyyaw_end[0] - xyyaw_start[0]) + ', '
                                + str(xyyaw_end[1] - xyyaw_start[1]) + ', '
                                + str(xyyaw_end[2] - xyyaw_start[2]) 
                                )
                 ...
                 # can get errors in the DDP planner. In that case, abort it and try again.
                    print("Unexpected error: " + str(sys.exc_info()[0]) + ' worker ' + str(worker_num))
                    traceback.print_exc()
                    print(' gave up on ' + urdf + ' plan ' + str(i_traj) + '/' + str(int(n_runs)))
                    time.sleep(0.1)
                    break
            ...
            print("Error: " + str(sys.exc_info()[0]) + ' worker ' + str(worker_num))
            traceback.print_exc()
            print(' gave up on ' + urdf + ' plan ' + str(i_traj) + '/' + str(int(n_runs)))
            time.sleep(0.1)
    ...
    # action_list state_list are both lists of divided np
    # mpc_save_dict is a dict()
    if not(show_GUI) and mpc_save_path is not None:
        mpc_save_dict['comment'] = ''
        mpc_save_dict['states_memory'] = states_memory
        mpc_save_dict['actions_memory'] = actions_memory
        mpc_save_dict['torques_memory'] = torques_memory
    ...
    torch.save(mpc_save_dict, mpc_save_path)
    print(' finished and saved ' + urdf + ' plans ')
...
if __name__ == '__main__':
    ...
    #                     target=plan_batch, 
    #                     args = (0, urdf, gnn_nodes_model, None,
    #                     [torch.device('cuda:0')], mpc_save_path,
    #                     1,  6,True,))
    # p.start()
    # p.join()