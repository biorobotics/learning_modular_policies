...
def apply_policy(urdf, goal_memory, 
    ...
    for i in range(len(goal_memory)):
        ...
        for t in range(run_len-1):
            ...
            if zero_out_last_states:
                node_inputs = create_control_inputs2([torch.zeros_like(s) for s in states],
                                                 states, goals_world)
            else:
                node_inputs = create_control_inputs2(last_states, states, goals_world)
