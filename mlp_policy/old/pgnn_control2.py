...
This version takes in the last state and current state observations
...
class Chassis(Node): 
    def __init__(self, internal_state_len, message_len, hidden_layer_size,
     goal_len=0, body_input=False):
        ...
        if body_input:
            input_len = input_len-3 # take out x,y,yaw
        input_len*=2 # add on last state
        input_len += goal_len # add on goal
...
class Wheel(Node): # sensors, actuators, ports
    ...
    def __init__(self, internal_state_len, message_len, hidden_layer_size):
        super().__init__("Wheel", internal_state_len, message_len,
            3*2, 2*2, 1, hidden_layer_size=hidden_layer_size)
...
class Leg(Node): # sensors 2, actuators 2, ports 1
    ...
    def __init__(self, internal_state_len, message_len, hidden_layer_size):
        super().__init__("Leg", internal_state_len, message_len,
         6*2, 3*2, 1, hidden_layer_size=hidden_layer_size)
...
def get_state_dicts(GNN_nodes):
    state_dict = dict()
    for GNN_node in GNN_nodes:
        state_dict[GNN_node.type] = GNN_node.state_dict()
    return state_dict
