...
def apply_policy(urdf, goal_memory, 
    ...
    gnn_nodes_control = gnn_nodes_in
    ...
    for i in range(len(goal_memory)):
        ...
        for t in range(run_len-1):
            ...
            with torch.no_grad():
                ...
                u_np = u_combined.cpu().numpy()
                u_div = env.divide_action_to_modules(u_np)
...
def make_goal_memory(n_steps, device=torch.device('cpu')):
    ...
    # speed_scale_xy = (T*dt)
    # these should match the ones from MPC_batch 
    # speed_scale_yaw = (T*dt)*1.1*(1./0.75)
    # speed_scale_xy = (T*dt)*0.314*(1./0.75) # these are drawn from planning_utils
