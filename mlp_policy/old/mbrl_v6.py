...
Currently I keep the current network weights in cpu, then
transfer them to gpu as needed, as in at the start of training,
and when spawning a new process that can benefit from gpu.
...
from MPC_batch_policy3 import plan_batch
from train_multidesign_control3 import train_control
from apply_policy3 import make_goal_memory, apply_policy

import pgnn_control as pgnnc

import pgnn
from train_multidesign_model import train_model

# import pgnn2 as pgnn
# from train_multidesign_model2 import train_model
...
if __name__ == '__main__':
    ...
    WRITE_FILES = False # flag to avoid generating log spam
    ...
    # num_mbrl_iters = 4 # number of alteration between plan and learning
    ...
    # urdf_names = ['llllll', 'wnwwnw'] # for testing
    # urdf_names = ['llllll', 'lwllwl','wnwwnw'] # for testing
    # urdf_names = ['llllll'] # for testing
    # urdf_names = ['wnwwnw'] # for testing

    start_time = datetime.now()
    start_time_str = datetime.strftime(start_time, '%Y%m%d_%H%M')
    # folder = 'mbrl_'+ start_time_str
    folder = 'mbrl_v8_test9_4'
    ...
    if w_tripod>0:
        print('Using tripod penalty')
    else:
        print('NOT using tripod penalty')
    print('Cost weights: ' + str(cost_weights))
    # print('Changed joint limits on wheels to pi/3')
    # print('Changed friction from test9')
    ...
    for urdf in urdf_names:
        ...
        print(urdf + ' measurement_stds: ' + str(measurement_stds[urdf]))

    print('foot friction: ' + str(env.foot_friction))
    print('wheel friction: ' + str(env.wheel_friction))
    print('plane friction: ' + str(env.plane_friction))
    ...
    num_rollouts_per_joint = 300.
    print('num_rollouts_per_joint: ' + str(num_rollouts_per_joint))
    # results in 150*100*NJ = 15,000*NJ = 120,000 - 270,000 data points
    ...
    for urdf in urdf_names:
        ...
        # if False: # force to recreate it
            ...
            print('Loading rollouts from ' + random_rollouts_fname)
            ...
            if USE_MULTIPROCESS: # Multiprocess version
                ...
                for p_num in range(num_processes):
                    ...
                    print(urdf + ' starting process ' + str(p_num) + 
    ...
    for urdf in urdf_names:
        ...
        if len(random_rollouts[urdf]['run_lens'])>=n_random_runs_used:
            ...
            print(urdf + 'num random runs: ' + str(len(random_rollouts[urdf]['run_lens'])))
    ...
    print('Starting dynamics learning')
    ...
    for urdf in urdf_names:
        ...
        # print(urdf + ' ' 
        #     + str(modules_types[urdf]) + ', ' 
        #     + str(attachments[urdf]))
    ...
    lr_init = 1e-3
    # lr_init = 5e-4
    optimizer_model = torch.optim.Adam(pgnn.get_GNN_params_list(gnn_nodes_model), 
                        lr=lr_init,
    ...
    for gnn_node in gnn_nodes_model: # share memory for later use by multiprocessing
        gnn_node.share_memory()

    # count number of parameters    
    num_nn_params=0
    for p in pgnn.get_GNN_params_list(gnn_nodes_model):
        nn=1
    ...
    if not(preload_model):
        ...
        train_model(model_fname, urdf_names, n_training_steps, 
                gnn_nodes_model, optimizer_model,
    ...
    ## create control GNN
    control_save_path = 'multidesign_control_iter0.pt'
    ...
    if preload_control:
        ...
        gnn_state_dict = pgnnc.get_state_dicts(gnn_nodes_control)
        save_dict_control = dict()
        save_dict_control['comment'] = 'initial network weights, untrained'
        save_dict_control['gnn_state_dict'] =  gnn_state_dict
        save_dict_control['internal_state_len'] = gnn_nodes_control[0].internal_state_len
        save_dict_control['message_len'] = gnn_nodes_control[0].message_len
        save_dict_control['hidden_layer_size'] = gnn_nodes_control[0].hidden_layer_size
...
#### ----- Main MBRL loop ----- ####
    ...
    for mbrl_iter in range(1,num_mbrl_iters+1):
        print('*** Starting mbrl iter ' + str(mbrl_iter) + ' ***')
        print_time()
        ...
        if mbrl_iter>1:
            gnn_nodes_control_input = gnn_nodes_control
            print('Using Policy for traj opt initial seed')
        else:
            gnn_nodes_control_input = None
            print('Using Zeros for traj opt initial seed')
        ...
        if not(USE_MULTIPROCESS):# or len(urdf_names)==1:
            print('running mpc serially')
            mpc_rollouts_now = dict()
            if mbrl_iter == num_mbrl_iters:
                # n_runs = 75 # need more data to train controller on last run
            ...
            for urdf in urdf_names:
                ...
                if not(os.path.exists(mpc_save_path)):
                    plan_batch(0, urdf, gnn_nodes_model, gnn_nodes_control_input,
                                        [backprop_devices[0]], mpc_save_path,
                                        n_envs = 7, n_runs = n_runs,
                                        show_GUI=False)
                                        # n_envs = 10, n_runs = n_runs,
            ...
            # if mbrl_iter == num_mbrl_iters:
            #     n_runs_per_process = 125 # use more data to train controller on last run
            # else:
            #     n_runs_per_process =  100
            ...
            print('n_envs_per_process: ' + str(n_envs_per_process))
            # print('n_runs_per_process: ' + str(n_runs_per_process))
            # n_envs_per_process = 7
            # results in 50*8*40 = 16,000 = 
            pool_inputs = []
            mpc_rollouts_now = dict()
            ind = 0
            for i in range(len(urdf_names)):
                ...
                if not(os.path.exists(mpc_save_path)):
                    pool_inputs.append([ind, urdf, 
                                    gnn_nodes_model, gnn_nodes_control_input,
                                    devices, 
                                    mpc_save_path,
                                    n_envs_per_process, 
                                    n_runs_per_process])
            ...
            if len(pool_inputs)>0:
                ...
                with torch.multiprocessing.Pool(processes=num_processes) as pool:
                    ...
                    # pool.starmap_async(plan_batch, pool_inputs)
        ...
        for urdf in urdf_names:
            ...
            print(urdf + ': ' + npstr(vm) + ' baseline ' + npstr(vm_baseline) + 
        ...
        # (unless this was the last iteration, in which case no need)
            ...
            if os.path.exists(model_fname):
                ...
                optimizer_model = torch.optim.Adam(
                    pgnn.get_GNN_params_list(gnn_nodes_model), 
                    lr=lr_init/4,
                    weight_decay= weight_decay)# create module containers for the nodes

                # for param_group in optimizer_model.param_groups:
                #     param_group['lr'] = lr_init/4
                #     print('learning rate: '+ str(param_group['lr']))
                # n_training_steps = 1250 + 250*mbrl_iter

                n_training_steps = 1000
                # frac_new_data = 0.05
                frac_new_data = 0.1

                print('Retraining model at iter ' + str(int(mbrl_iter)) 
                    + ',   frac_new_data = ' + str(frac_new_data))
                if USE_MULTIPROCESS:
                    print('Running train_model in parallel process')
        ...
        ### learn to clone plans into policy
        ...
        control_save_path = 'multidesign_control_iter' + str(int(mbrl_iter))+'.pt'
        control_save_path = os.path.join(folder, control_save_path)
        ...
        for urdf in urdf_names:
            ...
            print(urdf + ': ' + npstr(vm) + ' baseline ' + npstr(vmb) + 
                ', rescaled: ' + npstr(vm_rescaled))
